...
#define ESIZE 1 // Erro de tamanho do buffer
#define EINPUT 2 // Erro de input das variáveis
#define EOPEN 3 // Erro ao abrir o arquivo
#define ECLOSE 4 // Erro ao fechar o arquivo
#define EWRITE 5 // Erro ao escrever no arquivo
...
/**
 * @brief Copia uma instrução de 64 bits para um buffer de 8 bytes.
 *
 * Esta função recebe uma instrução de 64 bits e a copia para um buffer de 8 bytes.
 * A instrução é dividida em blocos de 8 bits e armazenada no buffer na ordem inversa.
 *
 * @param buffer Um ponteiro para o buffer onde a instrução será armazenada.
 * @param instruction A instrução de 64 bits a ser copiada.
 *
 * @return SUCCESS se a operação for bem-sucedida, ESIZE se o buffer não tiver espaço suficiente para armazenar a instrução.
 */
